...
import { ADDRESS_DIGI_REGISTRAR, ADDRESS_DIGI_REGISTRY } from '../config';
...
test.before(async (t: ExecutionContext<TestContext>) => {
  ...
  nockProvider(
    ...
    [
      {
        to: ADDRESS_DIGI_REGISTRAR.toLowerCase(),
        data: /^.*$/,
      },
      'latest',
    ],
  ...
  // fake vitalik.digi with nullifier
...
test('should raise an error if namehash of the name is not match with subgraph', async (t: ExecutionContext<TestContext>) => {
  ...
  t.is(
    error.message,
    'TokenID of the query does not match with namehash of vitalik.digi'
  );
  t.is(error.code, 404);
});
