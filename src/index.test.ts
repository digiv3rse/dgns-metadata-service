...
const handle21character = new MockEntry({
  name: 'handle21character.digi',
  registration: true,
  resolver: { texts: null },
});
const supercalifragilisticexpialidocious = new MockEntry({
  name: 'supercalifragilisticexpialidocious.digi',
  registration: true,
  resolver: { texts: null },
});
const longsubdomainconsistof34charactersMdt = new MockEntry({
  name: 'longsubdomainconsistof34characters.mdt.digi',
  registration: true,
  resolver: { texts: null },
});
...
test.before(async (t: ExecutionContext<TestContext>) => {
  ...
  nockProvider(
    ...
    [
      {
        to: ADDRESS_DIGI_REGISTRY.toLowerCase(),
  ...
  // something.digi recordExist true
  nockProvider(
    WEB3_URL,
    'eth_call',
    [
...
test('get /:contractAddress/:tokenId for domain (wrappertest3.digi)', async (t: ExecutionContext<TestContext>) => {
...
test('raise ContractMismatchError', async (t: ExecutionContext<TestContext>) => {
  ...
  t.assert(
    message ===
      `${NON_CONTRACT_ADDRESS} does not match with any DGNS related contract`
  );
});
